...
\begin{enumerate}
	\item Assumption of \tbf{realism}: it cannot be assumed that the physical properties $P_Q$, $P_R$, $P_S$ and $P_T$ have definite values $Q$, $R$, $S$ and $T$ which exist independent of observation.
	\item Assumption of \tbf{locality}: it cannot be assumed that the measurement performed by Alice does not influence the result of Bob's measurement.
\end{enumerate}

Together, these two assumptions are known as the assumptions of \tbf{local realism}. Clearly they seem to be plausible assumptions that match our everyday experience, yet the Bell inequalities show that at least one of the two is incorrect. To this day, it is now known if the world follows either one of them, but certainly the latter is definitly not \tit{locally realistic}. If anything, what the Bell inequality violation proves is that we know very little about entangled states and quantum physics in general.
...
Nevertheless, even if we cannot currently give a reasonable explaination of this outcome, we can still \todo{da finire}
