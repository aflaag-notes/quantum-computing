...
For example, if her two random strings are $$a = 01101$$ $$b = 00110$$ then she will construct the following superposition $$\ket \psi = \ket 0 + \ket 1 + \ket - + \ket + + \ket 1$$ Now, we will assume that Alice and Bob share two communication channels
...
\begin{itemize}
	\item Eve chooses the correct basis for $\ket \psi_i$: this means that she does not introduce any disturbance to the state Bob will receive, and Bob still has 50\% change of recovering Alice's original bit
...
However, we know that $U$ must be unitary, and by \cref{unitary alt def} this implies that $U$ preserves the scalar product, i.e.
\begin{equation*}
	\begin{alignedat}{2}
	...
	\end{alignedat}
\end{equation*}
This proves that there is no way for Eve to gain any information at all, but there is a subtlety to point out: the condition $\braket{\psi|\phi} \neq 0$ means that $\ket \psi$ and $\ket \phi$ must \tit{not be orthogonal}, and this is the only reason why the BB84 protocol works in the first place. Consider any two possible superpositions $\ket \psi$ and $\ket \phi$ Alice might send; since the Z and the X bases are not perfectly orthogonal it holds that $$\abs{\braket{0|+}}^2 = \abs{\braket{0|-}}^2 = \abs{\braket{1|+}}^2 = \abs{\braket{1|-}}^2 = \dfrac{1}{2} \neq 0$$ which means that its impossible that $\ket \psi$ and $\ket \phi$ are orthogonal. Indeed, without this crutial detail Eve could actually recover information of what Alice sent.
