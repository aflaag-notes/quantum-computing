...
Before explaining any protocol however, we need to talk about a theorem that we did not properly discuss in the first chapter \todo{section}, namely the \tbf{No-cloning theorem}. In particular, we stated that there is no quantum transformation that copies \underline{any} \tit{unknown} quantum state. Now that we have the mathematical tools to prove the theorem, we can actually show the real formulation of the theorem.
