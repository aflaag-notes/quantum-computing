...
For example, if her two random strings are $$a = 01101$$ $$b = 00110$$ then she will construct the following superposition $$\ket \psi = \ket 0 + \ket 1 + \ket - + \ket + + \ket 1$$ We observe that $\ket \psi$ in practice is treated as sequence of single qubits --- indeed this can be done in general whenever we are summing qubits that are not entangled between each other, such as $\ket{00} + \ket{11}$. Now, we will assume that Alice and Bob share two communication channels:
...
\begin{itemize}
	...
	\item if less that $t$ bits disagree, the error rate is estimated and used to apply \tit{error correction} algorithms to the key bits --- we will not cover the details of the error correction procedures
\end{itemize}
...
\begin{itemize}
	\item Eve chooses the correct basis for $\ket \psi_i$: this means that she does not introduce any disturbance to the state Bob will receive, and Bob still has 50\% chance of recovering Alice's original bit
	\item Eve chooses the wrong basis for $\ket \psi_i$: this means that she introduces some disturbance to the state Bob will receive, in the sense that now Bob has only 25\% chance of recovering Alice's original bit
\end{itemize}
...
\begin{itemize}
	...
	\item $U$ behaves differently depending on the input superposition, i.e. $\ket y \neq \ket{y'}$, otherwise she cannot distinguish between $\ket \psi$ and $\ket \phi$ upon measurement
\end{itemize}

Suppose that Alice sends some qubit which can be either $\ket \psi$ or $\ket \phi$, where $$\ket \phi, \ket \psi \in \{\ket 0, \ket 1, \ket + , \ket -\}$$ Since $U$ must be unitary, by \cref{unitary alt def} we know that $U$ preserves the scalar product, i.e.
\begin{equation*}
	\begin{alignedat}{2}
...
\begin{itemize}
	\item In the first scenario, the two possible qubits Alice might send are \tit{orthogonal}, i.e. $\braket{\psi|\phi} = 0$. Namely, these are the cases in which $\ket \psi = \ket 0$ and $\ket \phi = \ket 1$, and vice versa, and the cases in which $\ket \psi = \ket +$ and $\ket \phi = \ket - $, and viceversa. In these scenario the equality above is always satisfied, indeed Eve has a chance to recover some information without introducing disturbance. For instance, if Alice is sending either $\ket 0$ or $\ket 1$, a transformation $U$ such that $$U (\ket 0 \otimes \ket 0) = \ket 0 \otimes \ket 0$$ $$U (\ket 1 \otimes \ket 0) = \ket 1 \otimes \ket 1$$ is literally just a CNOT on $\ket 0$ where Alice's qubit is the control bit. In this case, if Eve guesses the Z basis (which happens with 50\% chance), she can measure the ancilla qubit and correctly deduce the value of Alice's qubit without introducing any disturbance at all. Nevertheless, this is really not an issue because
	      \begin{itemize}
		      \item Eve has no way to know if her choice was correct
		      \item if Eve chose the wrong basis, she introduces disturbance, which inevitably fails her plan
		      \item the BB84 protocol requires Alice to send a sufficiently large amount of qubits such that the possibilities of Eve being able to recover all the qubits by random chance is exponentially low
	      \end{itemize}
	\item In the second scenario, the two possible qubits Alice might send are \tit{non-orthogonal}, i.e. $\braket{\psi|\phi} \neq 0$. Let's see what happens to the previous equality:
	      \begin{equation*}
		      \begin{alignedat}{2}
			      \iff & \braket{\psi|\phi} \braket{y|y'} = \braket{\psi| \phi} \braket{x|x} &                                   \\
			      \iff & \braket{y|y'} = \braket{x|x}                                        & \quad (\braket{\psi|\phi} \neq 0) \\
			      \iff & \braket{y|y'} = 1                                                   &                                   \\
			      \iff & \ket{y} = \ket{y'}                                                  &                                   \\
		      \end{alignedat}
	      \end{equation*}
	      This is even worse for Eve: in this scenario, such $U$ cannot possibly exist, which means that there is no way for Eve to gain any information at all without introducing disturbance.
\end{itemize}
