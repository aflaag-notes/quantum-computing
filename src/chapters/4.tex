...
While algorithms like Grover and Shor's show the theoretical power of quantum computing, it must pointed out that they assume access to \tbf{large} and \tbf{error-free} quantum machines-hardware that does not yet exist. In fact, today's quantum devices are known as \tbf{Noisy Intermediate-Scale Quantum (NISQ)} machines, which have a limited number of qubits, noisy gates, short coherence times, and can only run shallow circuits before errors overwhelm the computation. In this section we will present a fairly recent approach that allows to work with such noisy machines, called \tbf{Variational Quantum Algorithms (VQAs)}. They are a type of quantum algorithms that are designed to work on NISQ devices we have today. By combining short, parameterized quantum circuits with classical optimization, VQAs allow the quantum computer to handle tasks it performs best --- such as exploring complex quantum states and measuring key properties --- assisted by a classical computer that iteratively adjusts the circuit parameters. This hybrid method tolerates noise and imperfections, making it possible to obtain useful results even with limited, error-prone hardware.

Variational Quantum Algorithms (VQAs) are a leading method for achieving quantum advantage on today’s noisy, intermediate-scale quantum (NISQ) devices. They work by combining shallow, parameterized quantum circuits—well-suited to noisy hardware—with classical optimization techniques that adjust those parameters. This hybrid quantum-classical strategy resembles machine-learning approaches like neural networks and helps manage NISQ constraints while avoiding the deep circuits required by fully fault-tolerant quantum algorithms.

VQAs have been explored for a wide range of quantum computing applications and may offer the best chance for near-term quantum advantage. However, they still face significant challenges—such as difficulties in training, limits in accuracy, and efficiency issues. The review discusses both the promising potential of VQAs and the obstacles that must be addressed to ultimately achieve true quantum advantage. Due to the inherent versatility of VQAs, there is a wide variety of different algorithmics structures with different levels of complexity. Nevertheless, most of VQAs share the same basic elements. A VQA begins with a clearly defined task and, if needed, relevant training data. The first step is to design a cost (loss) function (C) that captures what it means to solve the problem. Next, one chooses an ansatz—a parameterized quantum circuit with adjustable parameters $\mathbf \theta$. The algorithm then trains these parameters through a hybrid quantum-classical optimization loop, seeking the values $$\mathbf \theta^* = \argmin_{\theta}{C(\theta)}$$ that minimize the cost function. The quantum computer evaluates the cost (or its gradient), while a classical optimizer updates the parameters.

VQAs are highly flexible because they support task-oriented programming, making them suitable for nearly every major application envisioned for quantum computers. In fact, they are powerful enough to enable universal quantum computation. This section reviews the key application areas where VQAs have been developed and summarizes their current level of implementation, with Figure 3 providing an overview. Section V further discusses which of these applications may offer a path to achieving quantum advantage.

In chemistry and many other fields, solving large eigenvalue problems—such as determining molecular properties via the Schrödinger equation—quickly becomes classically intractable because the required computational resources grow exponentially with system size. Traditional approximate methods help but cannot achieve efficient, exact solutions for large systems.

Quantum computing offers a promising path forward. The standard quantum algorithm for eigenvalue estimation, quantum phase estimation (QPE), provides exponential speedups but requires long coherent evolution and extremely deep circuits, making it impractical for near-term devices.

The authors present an alternative: a variational quantum approach that avoids the long coherent runtimes required by QPE. They implement a reconfigurable photonic quantum processing unit capable of efficiently computing expectation values of a Hamiltonian. Coupled with a classical optimization routine, this variational method estimates eigenvalues and eigenvectors with far fewer coherence demands. As a result, it provides a more practical route toward quantum-enhanced computation on near-term hardware.



The Quantum Expectation Estimation (QEE) algorithm computes the expectation value of a Hamiltonian (H) for a given quantum state . Any Hermitian Hamiltonian can be expanded as a sum of tensor products of Pauli operators with real coefficients. Because expectation values are linear, evaluating $\abk{\mathcal H}$ reduces to summing the weighted expectation values of these individual Pauli terms.

If a Hamiltonian contains only a polynomial number of Pauli-term components—which is true for many important physical systems such as electronic structure Hamiltonians, Ising and Heisenberg models, and k-sparse Hamiltonians—then its expectation value can be computed efficiently. A quantum device can evaluate each Pauli-term expectation value efficiently, even when the overall Hamiltonian is an exponentially large $2^n \times 2^n$ matrix. Thus, QEE enables efficient estimation of otherwise intractable Hamiltonian expectation values using an (n)-qubit quantum state.


A classical approach to estimating the terms of a Hamiltonian would require optimizing many reduced states independently, but this quickly runs into the **N-representability problem**, which is QMA-hard and therefore intractable for both classical and quantum computers. The quantum method avoids this because quantum hardware naturally stores the full global quantum state without needing to reconstruct reduced states.

The expectation value of any Pauli-term tensor product can be estimated through local measurements on each qubit, performed in parallel with constant-time coherence requirements. Since these operators have bounded spectra, each term can be estimated to precision ( $\epsilon$ ) with ($O(|h_{\max}|^2, M, \epsilon^{-2}$)) measurement repetitions, where (M) is the number of Hamiltonian terms and ($h_{\max}$) the largest coefficient.

Compared with Quantum Phase Estimation (QPE), this variational-style expectation estimation requires many more repetitions—scaling as ($O(\epsilon^{-2}$) instead of $O(\epsilon^{-1}$)—and repeated state preparation. However, it dramatically reduces the required coherence time while still providing an exponential advantage over classical exact methods.

TODO \todo{da finire, sta roba è copiata con chat non so che c'è scritto}
...
% \begin{frameddefn}{Extremal Eigenvalue Problem (EEP)}
%     Given an Hermitian matrix, find its extremal eigenvalues.
% \end{frameddefn}
%
% First, by \cref{spectral thm} we know that the eigenvalues of any Hermitian matrix must be real, so the problem is well defined. Furthermore, consider any \tbf{Integer Linear Program (ILP)}
%
% \begin{figure}[H]
%     \centering
%     \[\begin{array}{cc}
...
% \end{figure}
%
% and suppose that the set of solutions $Y$ is finite, so we can assign a bitstring to each solution
