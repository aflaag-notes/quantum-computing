...
\begin{framedalgo}{Quantum Phase Estimation algorithm}
    ...
    \begin{algorithmic}[1]
        \Function{QuantumPhaseEstimation}{$U$, $\ket u$}
            \State $q_t \takes \ket u$ \Comment{$q_t$ is the $(t + 1)$-th register}
            \For{$k \in [0, t - 1]$}
                \State $q_k \takes H(q_k)$
                \State $q_k, q_t \takes U^{2^k}(q_k, q_t)$
            \EndFor
            \State $q_0, \ldots, q_{t - 1} \takes \mbox{QFT}^\dag(q_0, \ldots, q_{t - 1})$
