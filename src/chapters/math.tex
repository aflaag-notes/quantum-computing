...
We see that the first property is the \tit{linearity} of the scalar product, while the second property is usually referred to as \tit{conjugate-symmetry}. Scalar products are also called \tit{inner products}, and are used to define many other tools on top of the vector space considered.
...
\begin{frameddefn}{Hermitian product}
	Given a vector space $V$ defined over $\C$, the \tbf{Hermitian product} is defined as follows: $$\forall u, v \in V \quad \braket{u, v} = \sum_{i = 1}^n{\overline{u_i} v_i}$$
\end{frameddefn}
...
\begin{framedprop}[label={hermitian prod prop}]{}
    Given a scalar product vector space $V$, for any two vectors $u, v \in V$ it holds that $$\braket{u,v} = \braket{\overline v , \overline u}$$
\end{framedprop}

\begin{proof}
    Fix any two vectors $u, v \in V$; then, we have that
    \begin{equation*}
        \begin{split}
            \braket{u,v} & = \overline{\braket{v,u}} \\ 
                         & = \overline{\sum_{i = 1}^n \overline{u_i}v_i} \\ 
                         & = \sum_{i = 1}^n \overline{\overline{u_i} v_i} \\ 
                         & = \sum_{i = 1}^n \overline{\overline{v_i}} \cdot \overline{u_i} \\ 
                          & = \braket{\overline v,\overline u}
        \end{split}
    \end{equation*}
\end{proof}

We are finally ready to explain the \curlyquotes{braket} notation that we used from the beginning of the previous chapter. This notation was invented by the Nobel Prize in Physics \href{https://it.wikipedia.org/wiki/Paul_Dirac}{Paul Dirac}, and it works as follows: first, observe that the Hermitian product can be rewritten as follows $$\braket{u, v} = \rmat{\overline{u_1} \cdots \overline{u_n}} \rmat{v_1 \\ \vdots \\ v_n}$$ To be precise, this product would yield a $1 \times 1$ matrix, which can be interpreted as a scalar. Through Dirac notation, we will write $$\braket{u, v} = \braket{u | v}$$ where $\bra u$ is called \tbf{bra}, and $\ket v$ is called \tbf{ket} (as in \curlyquotes{bra-ket}). In other words, we have that $\ket v$ is just a regular column vector $v \in V$ $$\ket v = \rmat{v_1 \\ \vdots \\ v_n}$$ defined over some scalar product vector space $V$, while $\bra u$ is a \tit{linear map} that acts as follows: $$\funcmap{\bra \cdot}{V}{\overline V}{\rmat{u_1 \\ \vdots \\ u_n}}{\rmat{\overline{u_1} & \cdots & \overline{u_n}}}$$ We observe that from this very definition we can already see the power of the Dirac notation: by writing $$\bra u \cdot \ket v$$ we are writing the product between a row conjugated vector $u$ and a column vector $v$, which will ultimately yield $1 \times 1$ vector containing exatcly $\braket{u, v}$! Therefore, this allows us to write that $$\bra u \cdot \ket v = \braket{u|v}$$
...
Moreover, the Hermitian product induces a \tbf{norm}, which is defined as follows.
...
We observe that by \cref{hermitian prod prop} we immediately derive that $$\forall v \in V \quad \norm{v} = \norm{\overline v}$$

\begin{frameddefn}{Orthogonality}
    Given a scalar product vector space $V$, two vectors $u, v \in V$ are said to be \tbf{orthogonal} if $\braket{u|v} = 0$.
\end{frameddefn}

\begin{frameddefn}{Orthonormal basis}
    Given a scalar product vector space $V$, a basis $\{e_i\}_{i = 1}^n$ is said to be \tbf{orthonormal} if $$\forall i, j \in [n] \quad \braket{e_i | e_j} = \delta_{ij}$$ where $\delta_{ij} = \soe{ll}{1 & i = j \\ 0 & i \neq j}$ is called \tbf{Kronecker delta}.
\end{frameddefn}
...
\begin{equation*}
	...
	\end{alignedat}
\end{equation*}
Indeed, with the scalar product we can compute the projection of $u$ onto the $i$-th vector of the basis. Hence, we can rewrite the first equation as follows: $$\ket u = \sum_{i = 1}^n{\alpha_i \ket{e_i}} = \sum_{i = 1}^n{\braket{e_i|u} \ket{e_i}} = \sum_{i = 1}^n{ \ket{e_i} \braket{e_i|u}}$$ In particular, from the last equality by linearity of the scalar product we have that $$\ket u = \rbk{\sum_{i = 1}^n{\ket{e_i} \bra{e_i}}} \ket u \implies I = \sum_{i = 1}^n{\ket{e_i} \bra{e_i}}$$ which is a famous identity in quantum mechanics called \tbf{resolution of the identity}.
...
This property is incredibly useful, because it implies that the adjoint operator of $A$ is its transposed conjugate matrix. Most notably, for any column vector $\ket x$ it holds that $$\bra x = \ket x^\dag$$ which further underlines the power of Dirac notation.
...
The reason why we say that $A$ is \curlyquotes{positive} if it satisfies such property becomes obvious if we choose $v$ to be an eigenvector of $A$. Let $v$ be an eigenvector of $A$ associated to some eigenvalue $\lambda$ of $A$; then $$0 \le \braket{v|Av} = \bra v \lambda \ket v = \lambda \braket{v|v}$$ However, by definition of scalar product we know that $\braket{v|v} \ge 0$ for any $v$, which must imply that $\lambda \ge 0$ as well. In other words, if $A$ is positive all of its eigenvalues must be positive.

\begin{framedthm}[label={positive prop}]{}
...
At the beginning of the previous chapter we said that all quantum gates are \tit{unitary transformations}, but we did now provide a definition of \tbf{unitarity}.
...
\begin{framedlem}[label={xz yz lemma}]{}
...
\begin{framedthm}[label={unitary alt def}]{Unitary operators (alt. def.)}
...
\begin{proof}
    Let $U$ be a unitary operator, and fix any two vectors $x, y \in \mathcal H$; then, we have that $$\braket{Ux|Uy} = \braket{U^\dag Ux|y} = \braket{Ix|y} = \braket{x|y}$$ Moreover, we observe that if $$U^\dag U = UU^\dag = I$$ it must be that $$U^{-1} = U^\dag$$ which means that $U$ is invertible, i.e. bijective, and in particular it is surjective.

    Conversely, suppose that $U$ is a surjective unitary operator such that $$\forall x, y \in \mathcal H \quad \braket{Ux|Uy} = \braket{x|y}$$ First, we need to prove that $U^{-1}$ exists.

    \claim{
        $U$ is invertible.
    }{
        Since $U$ is required to be surjective we only need to prove that it is injective. By way of contradiction, suppose that $U$ is not injective, i.e. there are distinct $x_1, x_2 \in \mathcal H$ such that $Ux_1 = Ux_2$. Then, this implies that $$Ux_1 = U x_2 \iff U(x_1 - x_2) = 0$$ and since $x_1 \neq x_2$ it must be that $x_1 - x_2 \in \mathcal H$ is a non-zero vector in the kernel of $U$. However, by the second property of $U$ we have that $$\braket{x_1 - x_2| x_1 - x_2} = \braket{U(x_1 - x_2)| U(x_1- x_2)} = \braket{0|0} = 0$$ and by definition of scalar product $$\braket{x_1 - x_2|x_1 - x_2} = 0 \iff x_1 - x_2 = 0 \iff x_1 = x_2$$ which is a contradiction $\lightning$.
    }

    Now, fix a vector $y \in \mathcal H$; by surjectivity of $U$ we know that $$\exists z \in \mathcal H \quad y = Uz$$ which means that for any $x \in \mathcal H$ it holds that
    \begin{equation*}
        \begin{split}
            \braket{x|U^\dag y} & = \braket{Ux|y} \\ 
             & = \braket{Ux|Uz} \\ 
              & = \braket{x|z} \\ 
              & = \braket{x|U^{-1}y}
        \end{split}
    \end{equation*}
    Therefore, by the previous lemma we conclude that $$\forall y \in \mathcal H \quad \braket{x|U^\dag y} = \braket{x|U^{-1}y} \implies U^\dag y = U^{-1} y$$ which means that $U^\dag$ behaves as $U^{-1}$ on any vector of $\mathcal H$. This means that $U^\dag$ is $U$'s inverse, concluding that $U$ is indeed unitary.
\end{proof}
...
\begin{proof}
    Since $\overline{\overline U} = U$, it suffices to prove the direct implication. Let $U$ be a unitary operator; then $$(\overline U)^\dag \overline U = \overline{U^\dag} \overline U = \overline{U^\dag U} = \overline I = I$$ and analogously $$\overline U (\overline U)^\dag = \overline U \overline{U^\dag} = \overline{U U^\dag} = \overline I = I$$
\end{proof}
...
\begin{proof}
    Since $(U^T)^T = U$, it suffices to prove the direct implication. Let $U$ be a unitary operator; then $$(U^T)^\dag U^T = (U^\dag)^T U^T = (U U^\dag)^T = I^T = I$$ and analogously $$U^T (U^T)^\dag = U^T (U^\dag)^T = (U^\dag U)^T = I^T = I$$
\end{proof}
...
\begin{proof}
    Let $U$ be a unitary operator, therefore $U$ preserves the scalar product. In particular, we observe that $$\forall i, j \quad \braket{Ue_i|Ue_j} = \braket{e_i|e_j} = \delta_{ij}$$ and since $Ue_i$ and $Ue_j$ are the $i$-th and $j$-th columns of $U$ respectively, this immediately proves that $U$'s columns are orthonormal. To prove the same result for the rows, we first observe that $U^Te_i$ and $U^Te_j$ are the $i$-th and $j$-th rows of $U$, respectively. However, by the previous proposition we know that $U$ is unitary if and only if $U^T$ is unitary, which means that $$\forall i, j \quad \braket{U^T e_i | U^T e_j} = \braket{e_i|e_j} = \delta_{ij} $$ which immediately concludes the proof.
\end{proof}
...
In particular, we observe that the last property follows trivially from the cyclic property: $$\tr(UAU^\dag) = \tr(U^\dag UA) = \tr(I A) = \tr(A)$$ However, most importantly, the following property holds.
	...
	\begin{equation*}
		\begin{split}
			\tr(A \ket v \bra v) & = \sum_{i = 1}^n{\braket{e_i|A|v} \braket{v|e_i}}             \\
			                     & = \sum_{i = 1}^n{\braket{v|e_i} \braket{e_i|A|v}}          \\
...
\begin{framedprop}[label={broken trace}]{}
    Given a Hilbert space $\mathcal H$, for any two vectors $v, w \in \mathcal H$ it holds that $$\tr(\ket v \bra w) = \braket{w|v}$$
\end{framedprop}

\begin{proof}
    From the same observation pointed out in the previous proof we see that
    \begin{equation*}
        \begin{split}
            \tr(\ket v \bra w) & = \sum_{i = 1}^n \braket{e_i|v} \braket{w|e_i} \\ 
                               & = \sum_{i = 1}^n \braket{w|e_i} \braket{e_i|v} \\ 
                               & = \bra w \rbk{\sum_{i = 1}^n \ket{e_i} \bra{e_i}} \ket v \\ 
                               & = \braket{w|I|v} \\ 
                                & = \braket{w|v}
        \end{split}
    \end{equation*}
...
Interestingly enough, from this property we immediately derive that $$\tr( \ket v \bra v) = \braket{v|v}$$ which should not come as a surprise anyway, since the elements on the diagonal of $\ket v \bra v$ are exactly the elements of the sum in the scalar product of $\braket{v|v}$. We will discuss the importance of these equalities in the next chapter.

\begin{framedprop}{}
    Given a matrix $A \in \C^{n \times n}$, it holds that $$\tr(A^\dag)= \overline{\tr(A)}$$
\end{framedprop}

\begin{proof}
    By \cref{conj transp}, we observe that $$\forall i \in [n] \quad a_{ii}^\dag = \overline{a_{ii}}$$ which immediately proves that 
    \begin{equation*}
        \begin{split}ì
            \tr(A^\dag) & = \sum_{i = 1}^n a_{ii}^\dag \\ 
...
We show another very interesting fact of the trace in \cref{indep trace}.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item Let $A$ be a Hermitian operator, and let $v$ be an eigenvector of $A$ associated to an eigenvalue $\lambda$; then, we have that
            \begin{equation*}
                \begin{alignedat}{2}
                    & \braket{v|Av}  = \braket{Av|v} & \quad \quad (\mbox{by Hermiticity}) \\ 
                     \iff & \bra v \lambda \ket v = \bra v \overline \lambda \ket v \\ 
                     \iff & \lambda \braket{v|v} = \overline \lambda \braket{v|v} \\ 
                \end{alignedat}
            \end{equation*}
            Now, since $v$ is an eigenvector we know that $v \neq 0$, hence this implies that $\braket{v|v} \neq 0$, therefore we conclude that $$\lambda \braket{v|v} = \overline \lambda \braket{v|v} \iff \lambda = \overline \lambda$$
        \item Let $U$ be a unitary operator, and let $v$ be an eigenvector of $U$ associated to an eigenvalue $\lambda$; then, it holds that
            \begin{equation*}
                \begin{alignedat}{2}
                    \braket{v|v} & = \braket{Uv|Uv} & \quad \quad (\mbox{by \cref{unitary alt def}}) \\ 
                                 & = \bra{Uv} \lambda \ket v & \\ 
                                 & = \overline \lambda \bra v \lambda \ket v & \\ 
                                 & = \overline \lambda \lambda \braket{v|v} & \\ 
                                 & = \abs \lambda^2 \braket{v|v} & \\ 
                \end{alignedat}
            \end{equation*}
            Now, since $v$ is an eigenvector it must be that $v \neq 0$, therefore $\braket{v|v} \neq 0$ which means that $$\braket{v|v} = \abs \lambda^2 \braket{v|v} \iff \abs \lambda^ 2 = 1 \implies \abs \lambda = 1$$
        \item Let $A$ be a Hermitian operator, and let $v$ and $w$ be two eigenvectors associated to two distinct eigenvalues $\lambda \neq \mu$ of $A$; then, it holds that
            \begin{equation*}
                \begin{alignedat}{2}
                    \lambda \braket{v|w} & = \bra v \overline \lambda \ket w & \quad \quad (\mbox{by the first proposition}) \\ 
                                         & = \braket{Av|w} & \\ 
                                         & = \braket{v|Aw} & \quad \quad (\mbox{by Hermiticity}) &\\ 
                                         & =\bra v \mu \ket w & \\ 
                                         & = \mu \braket{v|w} & \\ 
                \end{alignedat}
            \end{equation*}
            Hence, we conclude that $$\lambda \braket{v|w} = \mu \braket{v|w} \iff (\lambda - \mu) \braket{v|w} = 0$$ and since $\lambda \neq \mu$ it must be that $\braket{v|w} = 0$. \todo{manca da dimostrare per le unitarie tbh}
    \end{enumerate}
\end{proof}
...
Interestingly enough, given a topologically closed subspace $V \subset \mathcal H$ of some Hilbert space, we can write any vector $u \in V$ as the sum of two orthogonal vectors of $V$ and $V^\bot$, as follows. Let $\{f_1, \ldots, f_n\}$ be an orthonormal basis of $V$, and define the following vectors $$\forall u \in U \quad u_V := \sum_{i = 1}^n{\braket{f_i|u} \ket{f_i}}$$ Then, if we call $$u_{V^\bot} := u - u_V$$ we see that we can actually show that that $u_V$ and $u_{V^\bot}$ are orthogonal to each other --- we will prove this fact in \cref{proj ex}. With this observation, we can finally define the projector operators.

\begin{frameddefn}{Projector}
    Given a Hilbert space $\mathcal H$, and a closed subspace $V \subset \mathcal H$, the \tbf{projector} operator that projects any given vector $v \in \mathcal H$ onto $V$ is defined as follows: $$\funcmap{P_V}{\mathcal H}{V}{u}{u_V = \sum_{i = 1}^n{\braket{f_i|u} \ket{f_i}}}$$ where $\{f_1, \ldots, f_n\}$ is an orthonormal basis of $V$.
\end{frameddefn}
...
\begin{proof}
	We first prove the direct implication. Suppose that $P_V$ is the projector of a closed subspace $V \subset \mathcal H$, meaning that $P_V$ can be written as $$P_V = \sum_{i = 1}^n \ket {f_i} \bra{f_i}$$ for some orthonormal basis $\{f_i\}_{i = 1}^n$ of $V$. Then, we observe that
	\begin{equation*}
		\begin{split}
...
\begin{proof}
    We provide a proof of this theorem in \cref{spectral decomp proof}.
\end{proof}
...
The last operation that we need to introduce into our mathematical background is the \tbf{tensor product}, and there are multiple ways to define it. We will first show the most general form, then we will see what version we will employ for our purposes.
...
\begin{framedprob}{}
    Let $\{f_i\}_{i = 1}^n$ be an orthonormal basis of a vector space $V$, and fix a vector $u \in V$; then, consider $$u_V := \sum_{i = 1}^n{\braket{f_i|u} \ket{f_i}}$$ Prove that the definition of $u_V$ does not depend on the choice of $\{f_i\}_{i = 1}^n$
\end{framedprob}
...
\solution{
    By Hermiticity, we know that $P$ admits a \nameref{spectral decomp}, i.e. $$P = \sum_{i = 1}^n{\lambda_i P_{\lambda_i}}$$ Now consider the following claim.

    \claim{
        If $P$ is an idempotent operator, its only possible eigenvalues are 0 and 1.
    }{
        By definition $v$ is an eigenvalue of $P$ associated to the eigenvector $\lambda$ if it holds that $$Pv = \lambda v$$ Hence, we observe that $$P^2 v = P (P v) = P (\lambda v) = \lambda Pv = \lambda(\lambda v) = \lambda^2 v$$ and therefore we have that $$\lambda^2 v = P^2v = Pv = \lambda v$$ which implies that $$\lambda^2v - \lambda v = 0 \iff (\lambda^2 - \lambda)v = 0$$ Finally, since $v$ is an eigenvector it holds that $v \neq 0$, therefore it must be that the last equation is true only if $$\lambda^2 - \lambda = 0 \iff \lambda = 0 \lor \lambda = 1$$
    }

    From this claim, we immediately obtain that each $\lambda_i$ is either 0 or 1, which means that its spectral decomposition can be written as $$P = \sum_j P_{\lambda_j}$$ for some non-zero eigenvalues $\lambda_j$ of $P$. Finally, since $P_{\lambda_j}$ are projectors onto different eigenspaces they are orthogonal between each other, which means that the statement follows immediately from \cref{projector sum}.
}
...
\solution{
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    Then, from the previous observation we have $$\forall x, y, z \in \mathcal H , \alpha, \beta \in \C \quad \braket{U(\alpha x + \beta y)|z} = \braket{\alpha Ux + \beta Uy|z}$$ and by \cref{xz yz lemma} we conclude that $$U(\alpha x + \beta y) = \alpha U x + \beta U y$$ meaning that $U$ is indeed linear.
}

\begin{framedprob}[label={spectral decomp proof}]{}
    Prove the \nameref{spectral decomp} theorem.
\end{framedprob}

\solution{
    TODO \todo{da finire la direct, decommenta}
    % Let $A$ be a normal operator in a Hilbert space $\mathcal H$. Then, by \cref{spectral thm 2} we know that the eigenvectors of $A$ form an orthonormal basis of $\mathcal H$, which must imply that $$I = \sum_{i = 1}^n{\ket{\lambda_i} \bra{\lambda_i}}$$ where each $\ket{\lambda_i}$ is a distinct eigenvector of $A$. Then, we conclude that
    % \begin{equation*}
    %     \begin{split}
    %         A & = A \cdot I \\ 
    %           & = A \sum_{i = 1}^n \ket{\lambda_i} \bra{\lambda_i} \\ 
    %           & = \sum_{i = 1}^n A \ket{\lambda_i} \bra{\lambda_i} \\ 
    %           & = \sum_{i = 1}^n \lambda_i \ket{\lambda_i} \bra{\lambda_i} \\ 
    %     \end{split}
    % \end{equation*}
    % Finally, even if $A$ might have degenerate eigenvalues, we observe that its eigenvectors 

    Now we will prove the converse implication. Let $A$ be an operator that can be written as $$A = \sum_i \lambda_i P_{\lambda_i}$$ where each $\lambda_i$ is an eigenvalue of $A$. Then, we have that
    \begin{equation*}
        \begin{split}
            A^\dag A & = \rbk{\sum_i \lambda_i P_{\lambda_i}}^\dag \rbk{\sum_j \lambda_j P_{\lambda_j}} \\ 
                     & = \rbk{\sum_i \overline{\lambda_i} P_{\lambda_i}} \rbk{\sum_j \lambda_j P_{\lambda_j}} \\ 
                     & = \sum_i \sum_j \overline{\lambda_i} \lambda_j P_{\lambda_i} P_{\lambda_j}
        \end{split}
    \end{equation*}
    and since projectors onto different eigenspaces are orthogonal between each other we have that
    \begin{equation*}
        \begin{split}
            A^\dag A & = \sum_i \sum_j \overline{\lambda_i} \lambda_j P_{\lambda_i} P_{\lambda_j} \\ 
                     & = \sum_i \overline \lambda_i \lambda_i P_{\lambda_i}^2 \\ 
                     & = \sum_i \abs{\lambda_i}^2 P_{\lambda_i} \\ 
        \end{split}
    \end{equation*}
    Finally, it is easy to see that computing $AA^\dag$ would yield the same result, proving that $A$ is indeed normal.
}

\begin{framedprob}{}
    Show that for any two complex numbers $z, w \neq 0$ it holds that $$\abs{\dfrac{z}{w}} = \dfrac{\abs z}{\abs w}$$
\end{framedprob}

\solution{
    Let $z, w \neq 0$ bet two complex numbers, and consider their polar forms $$z = r e^{i \theta} \quad w = s e^{i \varphi}$$ where $\theta, \varphi \in \R$ and $r = \abs z$, $s = \abs w$. Most importantly, we observe that $r, s > 0$ since $z, w \neq 0$. This implies that
    \begin{equation*}
        \begin{alignedat}{2}
            \abs{\dfrac{z}{w}} & = \abs{\dfrac{r e^{i \theta}}{s e^{e^{\varphi}}}} &  \\ 
                               & = \abs{\dfrac{r}{s} \cdot e^{i (\theta - \varphi)}} & \\ 
                               & = \dfrac{r}{s} \cdot \abs{e^{i (\theta - \varphi)}} & \quad \quad (\mbox{since $r, s > 0$}) \\ 
                               & = \dfrac{\abs z}{\abs w} & \quad \quad (\forall \alpha \in \R \quad \abs{e^{i \alpha}} = 1) \\ 
        \end{alignedat}
    \end{equation*}
}

\begin{framedprob}[label={prob ex}]{}
    Show that $u_V$ and $u_{V^\bot}$ are orthogonal.
\end{framedprob}

\solution{
    Recall that $$\forall u \in V \quad u_V := \sum_{i = 1}^n \braket{f_i|u} \ket{f_i}$$ for some orthonormal basis $\{f_i\}_{i = 1}^n$ of $V$. Then, we have that
    \begin{equation*}
        \begin{split}
            \braket{u_V|u_{V^\bot}} &= \braket{u_V | u - u_{V^\bot}} \\ 
                                    & = \braket{u_V | u} - \braket{u_V|u_V} \\ 
                                    & = \sum_{i = 1}^n \overline{\braket{f_i|u}} \braket{f_i|u} - \sum_{i =1}^n \overline{\braket{f_i|u}} \bra{f_i} \ket {u_V} \\
                                    & = \sum_{i = 1}^n \abs{\braket{f_i|u}}^2 - \sum_{i =1}^n \overline{\braket{f_i|u}} \bra{f_i}\rbk{\sum_{j = 1}^n \braket{f_j|u} \ket{f_j}} \\
                                    & = \sum_{i = 1}^n \abs{\braket{f_i|u}}^2 - \sum_{i = 1}^n \sum_{j = 1}^n \overline{\braket{f_i|u}} \bra{f_i} \braket{f_j|u} \ket{f_j } \\
                                    & = \sum_{i = 1}^n \abs{\braket{f_i|u}}^2 - \sum_{i = 1}^n \abs{\braket{f_i|u}}^2 \\
                                    & = 0
        \end{split}
    \end{equation*}
    which indeed proves that $u_V$ and $u_{V^\bot}$ are orthogonal.
}
