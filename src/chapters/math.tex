...
% We can now introduce a notation that will be used extensively throughout the notes. Given a vector space $V$, when we write $V^n$ what we mean is $$\underbrace{V\times \ldots \times V}_{n \ \mathrm{times}}$$ However, if we want to refer to $$\underbrace{V\otimes \ldots \otimes V}_{n \ \mathrm{times}}$$ we will denote this vector space as $V^{\otimes n}$. Then, the usual $\mathbb F ^{n \times  m}$ notaion will be reserved to matrices $n \times m$ defined over the field $\mathbb F$.
