...
\begin{frameddefn}{Tensor product}
	...
	The output space $V \otimes W$ is a vector space that has $$\mathcal B_{V \otimes W} := \{f_i  \otimes g_j \mid f_i \in \mathcal B_V, g_j \in \mathcal B_W\}$$ as a basis.
\end{frameddefn}

Consider any two vectors $v \in V$ and $w \in W$; we can describe them in terms of $\mathcal B_V$ and $\mathcal B_W$, respectively, as follows: $$v = \sum_i {\alpha_i f_i} \quad \quad w = \sum_j{\beta_j g_j}$$ Then, by the definition above the distributive properies imply that
\begin{equation*}
	\begin{split}
		v \otimes w & = \rbk{\sum_i{\alpha_i f_i}} \otimes \rbk{\sum_j {\beta_j g_j}} \\
		            & = \sum_i {\sum_j {\alpha_i \beta_j (f_i \otimes g_j)}}
	\end{split}
\end{equation*}
which correctly lies inside $V \otimes W$ since $(f_i \otimes g_j) \in \mathcal B_{V \otimes W}$
