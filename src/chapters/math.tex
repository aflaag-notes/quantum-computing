...
\solution{
    ...
    From this claim, we immediately obtain that each $\lambda_i$ is either 0 or 1, which means that its spectral decomposition can be written as $$P = \sum_j P_{\lambda_j}$$ for some non-zero eigenvalues $\lambda_j$ of $P$. Finally, since $P_{\lambda_j}$ are projectors onto different eigenspaces they are orthogonal between each other, which means that the statements follows immediately from \cref{projector sum}.
}
