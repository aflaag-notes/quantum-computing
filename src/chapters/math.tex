...
The last operator that we need to add into our mathematical background is the \tbf{tensor product}. There are multiple ways to define the tensor product.

\begin{frameddefn}{Tensor product}
	Given two vector spaces $V$ and $W$ defined over the same field, described by two bases $\mathcal B_V$ and $\mathcal B_W$, a \tbf{tensor product} $$\funcmap{\otimes}{V \times W}{V \otimes W}{(v, w)}{v \otimes w}$$ between $V$ and $W$ is a linear function that satisfies the following properties:
...
First, we observe that the output space $V \otimes W$ is a vector space that has $$\mathcal B_{V \otimes W} := \{f_i  \otimes g_j \mid f_i \in \mathcal B_V, g_j \in \mathcal B_W\}$$ as a basis. Additionally, from this definition we observe that $$\dim(V \otimes W) = \dim V \cdot \dim W$$ indeed $V$ and $W$ can have different dimensions.

To illustrate how the tenso product works, consider any two vectors $v \in V$ and $w \in W$, and suppose that $n := \dim V$ and $m := \dim W$. Then, we can describe them in terms of $\mathcal B_V$ and $\mathcal B_W$, respectively, as follows: $$v = \sum_{i = 1}^n{\alpha_i f_i} \quad \quad w = \sum_{j = 1}^m{\beta_j g_j}$$ Then, by the distributive properties in the definition we have that
\begin{equation*}
	\begin{split}
	...
	\end{split}
\end{equation*}
which indeed is a vector that lies inside $V \otimes W$ since $(f_i \otimes g_j) \in \mathcal B_{V \otimes W}$..

TODO \todo{commented part?}

% But first, we need to take a step back.
%
% Consider an operator that we are more accustomed to, the \tbf{cartesian product}. Given two sets $$A = \{a, b, c\}$$ $$B = \{1, 2, 3\}$$ the cartesian product computes all the possible combinations of the elements between $A$ and $B$:
%
% \begin{center}
% 	\begin{tabular}{c|ccc}
% 		\hline
...
% In particular, we use the tensor product when we need to describe functions that take more than 1 input on some domain. For instance, a 2-input function on the real number has the following signature $$\func{\R \times \R}{\R}$$ which means that the input of $f$ can be any pair $(x, y) \in \R^2$.
%
% When we introduced the tensor product in the first chapter, used to describe the first 2-input operator we encountered, i.e. the CNOT, and we stated that the tensor product was the correct operator to use instead of the usual cartesian product. There are two reasons why this is the case:
%
% \begin{itemize}
%     \item the tensor product \tit{combines} multiple vectors into one, so we essentialy send \tit{just one} input to the CNOT
%     \item the tensor product merges all the \tbf{multilinear interactions}
% \end{itemize}
%
% To illustrate the second point, consider theree vector spaces $V$, $W$ and $Y$; any function $$\func{V \times W}{Y}$$ takes 2 inputs, however this is not enough for vector spaces. In fact, must of the functions we deal with in vector spaces are \tit{linear}, and most importantly we often require functions to be linear in \tit{all its inputs}. The tensor product $V \otimes W$ 
%
% In fact, if $f$ is \tbf{bilinear}, w



TODO \todo{TODO pag 71 book}
