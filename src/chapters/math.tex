...
\begin{proof}
    Let $U$ be a unitary operator, any two vectors $x, y \in \mathcal H$; then, we have that $$\braket{Ux|Uy} = \braket{U^\dag Ux|y} = \braket{Ix|y} = \braket{x|y}$$ Moreover, we observe that if $$U^\dag U = UU^\dag = I$$ it must be that $$U^{-1} = U^\dag$$ which means that $U$ is invertible, i.e. bijective, and in particular it is surjective.

    Conversely, suppose that $U$ is a surjective unitary operator such that $$\forall x, y \in \mathcal H \quad \braket{Ux|Uy} = \braket{x|y}$$ First, we need to prove that $U^{-1}$ exists, and since $U$ is required to be surjective we only need to prove that it is injective. By way of contradiction, suppose that $U$ is not injective, i.e. there are distinct $x_1, x_2 \in \mathcal H$ such that $Ux_1 = Ux_2$. Then, this implies that $$Ux_1 = U x_2 \iff U(x_1 - x_2) = 0$$ and since $x_1 \neq x_2$ it must be that $x_1 - x_2 \in \mathcal H$ is a non-zero vector in the kernel of $U$. However, by the second property of $U$ we have that $$\braket{x_1 - x_2| x_1 - x_2} = \braket{U(x_1 - x_2)| U(x_1- x_2)} = \braket{0|0} = 0$$ and by definition of scalar product $$\braket{x_1 - x_2|x_1 - x_2} = 0 \iff x_1 - x_2 = 0 \iff x_1 = x_2$$ which is a contradiction $\lightning$.
...
\begin{framedprop}{}
    An operator $U$ is unitary if and only if $U^T$ is also unitary.
\end{framedprop}
...
In particular, we observe that the last property follow trivially from the cyclic property: $$\tr(UAU^\dag) = \tr(U^\dag UA) = \tr(I A) = \tr(A)$$ But perhaps most importantly, the following property holds.
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{split}
            \tr(\ket v \bra v) & = \sum_{i = 1}^n \braket{e_i|v} \braket{w|e_i} \\ 
...
Interestingly enough, from this property we immediately derive that $$\tr( \ket v \bra v) = \braket{v|v}$$ which should not come as a surprise anyway, since the elements on the diagonal of $\ket v \bra v$ as exactly the elements of the sum in the scalar product of $\braket{v|v}$. We will discuss the importance of these equalities in later sections.
