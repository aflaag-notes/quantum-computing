...
In the previous chapter, we explored some of the foundational algorithms of quantum computing, which demonstrated how quantum superpositions can \tit{outperform} classical approaches for specific problems. These algorithms, while conceptually simpler, help understanding the power and subtleties of quantum computation.

Building upon that foundation, we now turn our attention to a more sophisticated and central algorithm: \tbf{Quantum Phase Estimation (QPE)}. At its core, QPE allows us to determine the eigenvalues of a unitary operator with remarkable precision, a capability that lies at the heart of many quantum algorithms. Unlike the problems addressed by simpler algorithms, phase estimation tackles inherently more complex tasks and more broadly applicable, as we will see in the subsequent chapters.

The importance of QPE cannot be overstated. In this chapter, we will focus solely on QPE and how it works, while in the next chapters we will see how QPE is actually leveraged in the most widely known quantum algorithm, namely \tbf{Grover's algorithm} and \tbf{Shor's algorithm}.
...
\begin{frameddefn}{Principal $n$-th root of unity}
    Given an $n$-th root of unity $\omega$, we say that $\omega = e^{i \theta}$ is \tbf{principal} if and only if

    \begin{itemize}
        \item $\omega \neq 1$
        \item $\omega$ is $n$
        \item $\theta$ is minimal
    \end{itemize}
\end{frameddefn}

In other words, the principal $n$-th root of unity is the \curlyquotes{first} $n$-th root (after 1) that we encounter on the unit circle. Indeed, thanks to Euler's formula we usually define the principal $n$-th root of unity as $$\omega := e^{2 \pi i /n}$$ since $\tfrac{2 \pi}{n}$ is the $n$-th slice of the unit circle. Furthermore, the second condition of the definition is usually not provided in terms of order of the root, and it's written as follows $$\forall p \in [n - 1] \quad \sum_{j = 0 }^{n - 1}{\omega^{jp}} = 0$$ Aside from the geometric interpretation of this sum, this is a finite complex geometric series with common ratio $\omega^p$, which implies that
...
Therefore, we have that this sum is equal to 0 if and only if $\omega^p \neq 1$ and $\omega^{pn} = 1$. Indeed, when $\omega^p = 1$ since $p \in [n - 1]$ this would imply that the order of $\omega$ is less than $n$, meaning that $\omega$ was not a principal root.
...
\begin{proof}
    It satisfies to show that $$\mbox{QFT} ^\dag \ \mbox{QFT}  = \mbox{QFT} \ \mbox{QFT}^\dag  = I$$ We are going to prove the leftmost product first.
    \begin{equation*}
        % \hspace{-1.5cm}
        \begin{alignedat}{2}
            \mbox{QFT}^\dag \ \mbox{QFT} & = \sum_{j = 0}^{N - 1}{\ket j \rbk{\dfrac{1}{\sqrt N}\sum_{r = 0 }^{N - 1}{e^{-2 \pi i jr / N} \bra r}}} \sum_{k = 0}^{N - 1}{\rbk{\dfrac{1}{\sqrt N} \sum_{s = 0}^{N - 1}{e^{2 \pi isk/N} \ket s}} \bra k} & \\ 
                                       ...
                                       & = \sum_{j = 0}^{N - 1}{\ket j \bra j} + \dfrac{1}{N} \sum_{\substack{j, k = 0 : \\ j \neq k }}^{N - 1}{\ket j \rbk{\sum_{r = 0}^{N - 1}{e^{2 \pi i r(k - j)/ N}}}\bra k} & \\ 
    ...
    \begin{equation*}
        ...
        \end{alignedat}
    \end{equation*}
    Lastly, since $k - j$ is an integer for any $j, k \in [0, N - 1]$, therefore, the numerator is always equal to $1 - 1 = 0$, so the sum vanishes. Finally, we can conclude that $$\mbox{QFT}^\dag \ \mbox{QFT} = I + \dfrac{1}{N} \sum_{\substack{j, k = 0 : \\ j \neq k }}^{N - 1}{\ket j \rbk{\dfrac{1 - 1}{1 - e^{2\pi i (k - j)/ N}}}\bra k} = I$$ We will leave the second product as exercise.
\end{proof}
...
implying that it can be rewritten as follows $$\dfrac{1}{\sqrt N}\sum_{k = 0}^{N - 1}{\rbk{e^{2 \pi i k_n \cdot 0.x_1 \ldots x_n} \cdot \ldots \cdot e^{2 \pi i k_1 \cdot 0.x_n}}}\ket k$$ In other words the bits of $k$ are \tit{indexing} which exponentials to use in the $k$-th coefficient of the sum. This means that we can rewrite the last step as follows: $$\mbox{QFT} \ket x =  \dfrac{1}{\sqrt N} \bigotimes_{m = 1}^n{\rbk{\ket 0 + e^{2\pi i 0.x_m \ldots x_n} \ket 1}}$$ This formulation of the QFT is remarkable, because

\begin{itemize}
    \item it highlights exactly the goal of the Fourier transform: we encoded the information of $\ket x$ inside the \tit{phase} of the output
    \item the QFT written in this form can be actually implemented in a quantum circuit
\end{itemize}

We will now construct the QFT circuit. Consider the following operator: $$R_k := \rmat{1 & 0 \\ 0 & e^{2 \pi i /2^k}}$$ It can be easily proven that it is unitary, so we can use it as a quantum gate. Hence, we construct the following quantum circuit:

\begin{figure}[H]
	\[
		\Qcircuit @C=0.8em @R=1.3em {
                & \lstick{\ket{x_1}} & \gate H & \gate{R_2} & \gate \ldots & \gate{R_{n - 1}} & \gate{R_n} & \qw & \qw & \qw & \qw & \qw & \qw & \qw & \qw & \qw & \qw \\ 
                & \lstick{\ket{x_2}} & \qw & \ctrl{-1} & \gate \ldots & \qw & \qw & \gate H & \gate{R_2} & \gate \ldots & \gate{R_{n - 2}} & \gate{R_{n - 1}} & \gate \ldots & \qw & \qw & \qw& \qw \\ 
                & \lstick{\vdots} & & & \vdots & & & & & & & & \ldots \\
                & \lstick{\ket{x_{n - 1}}} & \qw & \qw & \qw & \ctrl{-3} & \qw & \qw & \qw & \qw & \ctrl{-2} & \qw & \gate \ldots & \gate H & \gate{R_2} & \qw & \qw \\ 
                & \lstick{\ket{x_n}} & \qw & \qw & \qw & \qw & \ctrl{-4} & \qw & \qw & \qw & \qw & \ctrl{-3} & \gate \ldots & \qw & \ctrl{-1} & \gate H & \qw \\ 
		}
	\]
	\caption{The circuit of the Quantum Fourier Transform.}
\end{figure}

TODO \todo{explain what is ts, non manca lo swap alla fine tipo?}

\section{Quantum Phase Estimation} \label{qpe section}

The application of the QFT we are going to present in this section was introduced by \textcite{qpe} in 1995, and it is the most important quantum algorithm \tit{to date}. As we will see, it will be used for multiple algorithms in the next chapters.

By \cref{spectral thm} we know that the eigenvalues of a unitary operator are complex values of modulus 1. This means that any eigenvalue of a unitary operator can be ewritten as $e^{2 \pi i \varphi}$ for some real value $\varphi \in [0, 1]$ --- $\varphi$ being the \tit{phase} of the complex value, i.e. the angle w.r.t. the unitary circumference.
...
In particular, we observe that the evaluation of the cost depends on the value of $\abk{H}$, and the best way we have to evaluate this quantity is by repeatedly measuring $H$. However, due to the size of $H$ and the noise of the machines this step is often implemented with a better alternative. In fact, there is a very useful property of Hamiltonians that we can leverage: any Hamiltonian can be written as linear combination of tensor products of Pauli matrices $$H = \sum_{j}{h_j \bigotimes_{k = 1}^n{\sigma_{\alpha_k, jk}}}$$ where $\alpha_k \in \{x, y, z\}$ --- we observe that the tensor product of $n$ matrices $2 \times 2$ yields a matrix of size $2^n \times 2^n$, as expected. The original approach of the VQE only considers Hamiltonians that can be written as a sum of a polynomial number of terms. This property has a very practical implication: by linearity of expectations it holds that $$\abk H = \sum_{j = 1}{h_j' \abk{\bigotimes_{k = 1}^{n}{\sigma_{\alpha_k, jk}}}}$$ which means that in order to compute the cost $C(\theta)$ we can instead evaluate the following $$C(\theta) = \abk H = \braket{\psi(\theta)| H \psi(\theta)} = \sum_{j = 1}{h_j' \abk{\bigotimes_{k = 1}^{n}{\sigma_{\alpha_k, jk}}}}$$ This is much more feasible on current NISQ hardware, since the tensor producto of Pauli matrices can be treated as $n$ different qubits $$\abk{\bigotimes_{k = 1}^{n}{\sigma_{\alpha_k, jk}}} = \abk{\sigma_{\alpha_1, j1} \otimes \ldots \otimes \sigma_{\alpha_n, jn}}$$ therefore measuring this expected value requires shallower circuits than computing the whole $\abk{H}$ which has size exponential size. We observe that this is an oversemplification and we are glossing over a lot of details, both from a theoretical and physical point of view, but such specifics are definitely outside the scope of this discussion as they would require an entire chapter on their own.
