...
Now, given any unitary transformation $U$, our next goal is to understand how to implement C-$U$ using only single qubit operations and the CNOT gate, leveraging the result above. \todo{TALK ABOUT THE FACT THAT WE NEED AN ENTANGLING QUBIT} Consider two qubits $q_0$ and $q_1$, and construct the operators $A$, $B$ and $C$ as described by the corollary. Suppose that $q_0$ is the control qubit; then, the controlled-$U$ operator should then work as follows:
...
TODO \todo{how to transform that thing universally now?}

\subsection{Multiple qubit conditioning}
...
\begin{figure}[H]
	\[
		...
		& \lstick{\ket{c_2}}    & \ctrl{4} & \qw & \qw & \qw & \qw & \qw & \qw &  \ctrl{4} & \qw \\
		& \lstick{\ket{c_3}}    & \qw & \ctrl{3} & \qw & \qw & \qw & \qw & \qw & \ctrl{3} & \qw & \qw \\
		& \lstick{\ket{c_4}}    & \qw & \qw & \qw & \qw & \qw & \qw & \qw & \qw & \qw  & \qw \\
		& \lstick{\ket{c_5}} \\
		& \lstick{\ket 0} & \targ & \ctrl{1} & \qw & \qw & \qw & \qw & \qw & \qw & \qw \ctrl{1} & \targ & \qw \\
...
the third wire is $\ket 1$ only if both the control qubits are 1.

TODO \todo{decomment this part}
