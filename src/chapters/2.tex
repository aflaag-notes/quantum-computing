...
\begin{equation*}
	...
	\end{split}
\end{equation*}
We observe that the matrix we used is derived as follows: $$A = 0 \cdot P_0 + 1 \cdot P_1 = 0 \cdot \ket 0 \bra 0 + 1 \cdot \ket 1 \bra 1$$ Notice that to evaluate this proability we did not use any particular assumption on the choice of the basis, so in general whenever the projector $P_\lambda$ has rank 1, i.e. $P_\lambda$ can be written as $$P_\lambda = \ket \lambda \bra \lambda$$ we can always directly write that $$\Pr[\mbox{measure}(\ket \psi = \ket \lambda)] = \abs{\braket{\psi|\lambda}}^2$$ This formulation of the probability will be used extensively for our purposes, and allows us to avoid the description of the matrix $A$ completely.
