...
\begin{framedpost}{Measurement postulate}
...
For instance, if we have a superposition $$\ket \psi = \alpha \ket 0 + \beta \ket 1$$ and we want to know what is the probability that $\ket \psi$ collapses to $\ket 0$ after a measurement, we have that
\begin{equation*}
	\begin{split}
		\Pr[\mbox{measure}(\ket \psi = \ket 0)] & = \Pr[A = 0 | \ket \psi]                   \\
		                                        & = \Pr \sbk{\rmat{0                     & 0 \\ 0 & 1} =0 \middle| \ket \psi} \\
		                                        & = \braket{\psi|P_0 \psi}                   \\
		                                        & = \braket{\psi|(\ket  0 \bra 0) \psi }     \\
		                                        & = \braket{\psi|\ket 0 \bra 0 \psi}         \\
		                                        & = (\braket{\psi|0})(\braket{0|\psi})       \\
		                                        & = \abs{\braket{\psi|0}}^2
	\end{split}
\end{equation*}
We observe that the matrix we used is derived as follows: $$A = 0 \cdot P_0 + 1 \cdot P_1 = 0 \cdot \ket 0 \bra 0 + 1 \cdot \ket 1 \bra 1$$ Notice that to evaluate this proability we did not use any particular assumption on the choice of the basis, so in general whenever the projector $P_\lambda$ has rank 1, i.e. $P_\lambda$ can be written as $$P_\lambda = \ket \lambda \bra \lambda$$ we can always directly write the following.

\begin{framedprop}{}
	The probability that a qubit $\ket \psi$ collapses to $\ket \lambda$ when measured is $$\Pr[\mbox{measure}(\ket \psi = \ket \lambda)] = \abs{\braket{\psi|\lambda}}^2$$
\end{framedprop}

We observe that this proposition is a direct consequence of \cref{spectral thm 2}, because it implies that the set of all the eigenvectors $\ket \lambda$ of the matrix $A$ we would have to consider always form an orthonormal basis of the complete Hilbert space, so we can measure any state w.r.t. them. This formulation of the probability of measurements will be used extensively for our purposes, and allows us to avoid the description of the matrix $A$ completely.
